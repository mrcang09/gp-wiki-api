# ASceneCaptureCube

- Symbol Type: class
- Symbol Path: Others / ASceneCaptureCube
- Source JSON Path: class/detail/Others/ASceneCaptureCube.json
- Source JSON URL: https://developer.gp.qq.com/api/class/detail/Others/ASceneCaptureCube.json
- Mirrored At (UTC): 2026-05-19 08:23:21Z

---

## Variables

| Name | Type | Description | Redirect |
| --- | --- | --- | --- |
| CaptureComponentCube | USceneCaptureComponentCube * | Scene capture component. |  |
| DrawFrustum | UDrawFrustumComponent * | To allow drawing the camera frustum in the editor. |  |

## Functions

### OnInterpToggle

Parameters:

| Name | Type | Description | Redirect |
| --- | --- | --- | --- |
| bEnable | bool |  |  |

Returns:

| Name | Type | Description | Redirect |
| --- | --- | --- | --- |
|  | void  |  |  |
